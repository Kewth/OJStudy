...
限制条件就是 2-SAT 中的若 a=0/1 则 b=0/1 ，
那么题目要求的就是 2-SAT 满足字典序要求的一组解。
枚举答案串与 s 的 lcp 长度 x ，
那么确定了前 x 个变量，然后后面的变量都有最多两个选择，贪心选字典序最小的一组。
需要预处理传递闭包，做到每次求解的复杂度在 O(n^2) 以内，
总复杂度 O(n^3) 。
其中 2-SAT 中若 a 则 b 不仅要连边 a -> b ，还要连边 !b -> !a 。
#endif
#include <cstdio>
#include <cstring>
#include <vector>
