 ...
 * 图染色
  ...
  考虑如果颜色 x, y 之间存在奇环，那么该奇环上的边可以分为两类，一类是两端同色的边，一类
是两端异色的边，其中前者有奇数个后者有偶数个。把所有同色边先拿出来建图，然后二分图染色。
下文称每个点二分图染色的结果颜色为其权值（0 或 1）。

  对于异色边，定义其权值为其两个端点的权值的异或和。那么颜色 x, y 之间的环是奇环当且仅当
环上的异色边的异或和为 1 。这个等价于奇环判断，判断这个再染一次色就好了。
