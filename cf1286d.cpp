...
首先第一次碰撞一定出现在两个相邻的点。
对于每对相邻点，求出所有碰撞情况下它们发生碰撞的时间，
得到 O(n) 个时间点进行排序。
那么第 i 个碰撞是第一次碰撞要求前 i - 1 次都没有碰撞，
设 p[i] 表示前 i 个碰撞都没有发生的概率，
那么第 i 个碰撞是第一个发生的概率就是 p[i - 1] - p[i] ，问题转换为求 p 。

用线段树维护 p ，具体地，如果当前要求 p[i] ，
线段树每个区间 [L, R] 维护四个值，
分别表示 L 向左/右 R 向左/右且 [L, R] 之间排在前 i 的碰撞都不发生的概率。
特别的是节点的左右儿子是 [L, mid] 和 [mid, R] ，两者有一个公共点，
而叶子节点就是形如 [k, k + 1] 的区间。

那么 p[i] 的值就是根的四种值的和，考虑从 p[i - 1] 到 p[i] 时怎么维护，
相当于钦定第 i 个碰撞不能发生，找到第 i 个碰撞对应的叶子节点，
将对应的碰撞方向的概率设为 0 ，然后向上更新即可。
#endif
#include <cstdio>
#include <cstring>
#include <algorithm>
