...
根据容斥原理，每个放点方案对答案的贡献是 -1^|S| ，
其中 S 是不满足条件的点的集合。
设 f[i][0/1] 表示 n = i 时，所有 |S| 为奇/偶的方案，
枚举最后一段连续的不满足条件的点的长度，预处理转移即可。

至于预处理转移，设 g[i][j] 表示转移的长度为 i ，其中最大的数为 j 的系数。
...
int main() {
	...
	for(int i = 1; i <= n; i ++)
		for(int j = 1; j <= i and j < maxl; j ++) {
			(f[i][0] += h[j] * f[i - j][1] % mod) %= mod;
			(f[i][1] += h[j] * f[i - j][0] % mod) %= mod;
		}
	printf("%lld\n", (f[n][n & 1] - f[n][!(n & 1)] + mod) % mod);
}
