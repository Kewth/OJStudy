...
2020.05.08
  ...
  考虑排序后的数列 {a} ，每一对数 a[i] 和 a[j] (i < j) 对答案的贡献是独立的，就是
a[i] a[j] / 2^(j - i + 1) 。如果没有修改，可以排序后前缀和去求，但众所周知前缀和的
思想难以放在修改上，从另一个角度考虑，用分治去做，分治结构可以很好支持修改，用线段
树维护分治树即可，由于分治的是排序后的序列，线段树也要用权值线段树去做。
#endif
#include <cstdio>
#include <algorithm>
#include <vector>
