...
int main() {
	...
	for(int b = 0; b < B; b ++) {
		for(int i = 1; i < B; i ++) {
			add(a[b * B + i - 1], 1);
			pre[b * B + i] = pre[b * B + i - 1] + i - query(a[b * B + i]);
		}
		for(int i = 1; i < B; i ++)
			add(a[b * B + i - 1], -1);
		for(int i = B - 2; i >= 0; i --) {
			add(a[b * B + i + 1], 1);
			suf[b * B + i] = suf[b * B + i + 1] + query(a[b * B + i] - 1);
		}
		for(int i = B - 2; i >= 0; i --)
			add(a[b * B + i + 1], -1);
		f[b][b] = suf[b * B];
	...
	while(q --) {
		...
		if(l / B == r / B) {
			...
			if(r - b * B < 40) {
				...
				for(int i = l; i <= r; i ++)
					tmp[++ p2] = a[i];
				std::sort(tmp + 1, tmp + p1 + 1);
				std::sort(tmp + B + 1, tmp + p2 + 1);
			}
			else
				for(int i = 0; i < B; i ++)
					if(second[b][i] < l)
