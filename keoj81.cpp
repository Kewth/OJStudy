...
黑科技，O(V) 预处理 + O(1) GCD 以及 O(sqrt(mod)) 预处理 + O(1) 快速幂。
神奇的是它们都与根号扯上关系。
...
每个 a^k 的 k 在模 mod - 1 意义下都可以表示为 a * sqrt(mod) + b ，
满足 a, b 都不超过 sqrt(mod) ，预处理即可。
#endif
#include <cstdio>
#include <algorithm>
...
const int maxn = 5050, maxv = 1000005, maxb = 1005, mod = 998244353, maxbm = 32000;
int a[maxn], b[maxn];
int gcd[maxb][maxb];
int magic[maxv][3];
int minp[maxv];
lolong powera[maxbm], powerb[maxbm];
...
lolong power(lolong k) {
	k %= mod - 1;
	return powera[k / maxbm] * powerb[k % maxbm] % mod;
}
...
int main() {
	...
	for(int i = 1; i < maxbm; i ++)
		powerb[i] = (powerb[i - 1] << 1) % mod;

	powera[0] = 1;
	for(int i = 1; i < maxbm; i ++)
		powera[i] = powera[i - 1] * powerb[maxbm - 1] * 2 % mod;
	...
	for(int i = 1; i <= n; i ++) {
		lolong ans = 0;
		for(int j = 1; j <= n; j ++)
			ans += power(1ll * ((i - 1) * n + j) * GCD(a[i], b[j]));
		ans %= mod;
		printf("%lld\n", ans);
	}
}
