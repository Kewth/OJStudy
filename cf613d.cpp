...
建虚树 DP ，设 f[u] 表示忽略 u 的父亲时 u 子树的最小代价，
g[u] 表示 u 向上需要删点时 u 子树和 u 向上的最小代价。
转移分两种，如果 u 是关键点那么只能将转移的地方 g 求和，
否则可以有一个地方将 g 改成 f ，或者 f 求和后加 1 （删掉 u 自己）。
事实上本质上 f, g 的区别是在做一个换根 DP 。
#endif
#include <cstdio>
#include <algorithm>
