...
棋盘博弈套套个矩形并这个思路感觉已经出烂了。。。
点 (x, y) 的 sg 值就是 min(lowbit(x), lowbit(y), greatbit(k)) 。
那么套在矩形并上，问题就是怎么求一段区间的 sg 值的情况，
以及怎么把两个维度的 sg 值合并。
注意到 lowbit, greatbit 的一共只有 O(logV) 种取值，
求出每种取值的数量即可，两个维度合并直接暴力在两边枚举。
#endif
#include <cstdio>
#include <algorithm>
#include <cassert>
