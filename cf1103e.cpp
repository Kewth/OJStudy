...
2020.05.06
  ...
  设 f[x] 表示有多少数是 x ，那么答案要求的就是 f 的 n 次高维循环卷积。
  但是模数比较特殊，有两个特别的地方：
  1) 底层 FFT 需要 10 次单位根 W ，在模数意义下不存在这个 W ，需要扩域，扩 5 维即可，
因为有 W^5 = -1 。
  2) IDFT 后需要把所有数除以 1e5 ，在模数意义下这个不存在逆元，但是可以拆成 2^5 和
5^5 ，后者有逆元，前者乘到模数里面即可。
#endif
#include <cstdio>
#include <cstring>
#include <algorithm>
