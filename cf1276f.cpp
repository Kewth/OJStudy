...
把要统计的串分为以下几类：s, s*t, s*, *s, * 。
重点考虑统计 s*t 的数量。

考虑枚举 s ，统计对应的 t 的数量。
不难发现对于 endpos 集合相同的 s ，其 t 的数量是相同的。
那么可以在 SAM 上枚举 O(n) 个节点来计算 t 的数量。
设 s 的 endpos 集合为 X ，设集合 Y = {y=x+2|x in X} ，
那么把起点在 Y 集合的所有后缀拿来建 Trie ，Trie 的节点数量就是合法 t 的数量。
众所周知求 Trie 的节点数量可以子集容斥，但是复杂度太大。
另一个方法是按字典序插入字符串，设上一个插入的串为 m ，插入 n 后，
trie 树的节点数量会增加 |n| - |lcp(n, m)| 。
也就是说只要把起点在 Y 集合的后缀按字典序排序并且要求相邻 lcp 的和。
这显然就是要后缀排序后求相邻 height 最小值的和。
在 parent 树上通过 set 启发式合并维护 Y 集合。
启发式合并的同时可以维护 trie 树大小。
#endif
#include <cstdio>
#include <algorithm>
#include <cstring>
