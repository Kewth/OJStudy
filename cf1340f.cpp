...
2020.05.05
  ...
  我他妈裂开啦。
  用线段树维护括号序列，考虑合并两个括号序列 A, B 。如果 A 或者 B 出现了匹配错误的情况，
那么合并后一定是匹配错误的。否则，A, B 把匹配了的括号删掉后只会剩前面一段右括号加上后面
一段左括号。将 A 的右边和 B 的左边进行匹配，如果匹配成功就能合并。
  在线段树节点内用可持久化 Treap 维护两段右括号和左括号即可，Treap 内维护哈希，这样就能
快速判断两段是否匹配。
  卡空间，需要回收节点。
#endif
#include <cstdio>
