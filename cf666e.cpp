...
挺板的，所有串拉出来建广义 SAM ，然后每个节点维护一颗线段树，
线段树第 i 个叶子节点存了该节点在 T[i] 的出现次数，
在 parent 树上线段树合并即可。
对与查询，找到 S 的第 pr 个前缀所在的节点，
然后在 parent 树上跳父亲边，需要满足 len >= pr - pl + 1 ，
跳到不能跳就找到了 S[pl:pr] 所对应的节点，从而在线段树上查询 [l, r] 的答案。
但是直接跳的复杂度没有保证，倍增即可。
#endif
#include <cstdio>
#include <algorithm>
