...
将一条路径的权值 ax + b 看做二维平面上的一个点 (a, b) 。
那么询问为 k 时该点的答案就是斜率为 -k 的直线截该点的截距。
对于所有可能的 (a, b) ，询问一定是截在它们的凸包上，考虑求出这个凸包。

边分治统计路径，对于中心边，分别求出其两侧的所有路径代表的点 (a, b) ，
然后分别作出凸包 A, B ，特别地中心边自身算在 A 里面，
有效的每条经过中心边的路径一定是 A, B 的两个点加起来。
也就是说 A, B 的闵科夫斯基和就是经过中心边的路径的权值的凸包。
求出所有这样的凸包，然后全部仍在一起再求一个凸包即可得到最终的凸包。
#endif
#include <cstdio>
#include <algorithm>
#include <vector>
