...
观察 DFS 过程中 ff[u], gg[u] 的变化，
其中 ff 为 u 到 1 上 1 ~ x 的子序列数，gg 是 DFS u 之前所有 DFS 结束的点 x ~ L 的子序列数。
...
考虑如何处理修改，利用到 f[u] < u 以及按顺序修改这两个性质，
考虑每 n 个修改看做一轮，不难发现：
1) 这一轮 u 点修改前，u 的祖先一定已经修改，
2) 这一轮 u 的子树修改前，u 点一定已经修改。
那么每一轮记录修改前和修改后的 val 并按上述方法求出两个值的 f, g ，
那么点 u 修改后的 ans[u] 就是
ans[u - 1] + f[u][new[x] - 1] * g[u][new[x] - 1] - f[u][old[x] - 1] * f[u][old[x] - 1] ，
